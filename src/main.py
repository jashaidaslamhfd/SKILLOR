...
class SKILLORPipeline:
    ...
    def run_pipeline(self, topic: str = None, schedule_publish: bool = True) -> dict:
        ...
        if not image_paths:
            raise RuntimeError(
                "Koi bhi image nahi bani (Pollinations, Hugging Face, Gemini, placeholder sab fail) — "
                "pipeline rok rahe hain. HF_API_KEY / GEMINI_API_KEY aur assets/placeholder.png check karo."
            )
