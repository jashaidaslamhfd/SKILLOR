...
class YouTubeUploader:
    ...
    def upload_video(self, video_path: str, thumbnail_path: str, title: str,
        ...
        # FIX: thumbnail_path can legitimately be None (main.py passes None
        # when thumbnail generation fails upstream so the video can still
        # upload without one). The old code called os.path.abspath(None)
        # unconditionally here, which raises TypeError before the
        # os.path.exists() check below ever runs -- crashing the entire
        # upload (and therefore the whole pipeline) even though a missing
        # thumbnail should never be a fatal error.
        ...
        if abs_thumbnail_path and not os.path.exists(abs_thumbnail_path):
            print(f"  ⚠️ Thumbnail not found, continuing without one: {abs_thumbnail_path}")
        ...
        # FIX: only attempt the thumbnail upload step if we actually have
        # one -- previously this always ran with abs_thumbnail_path, which
        # could be None and would crash MediaFileUpload deep inside
        # upload_thumbnail_with_retry.
        if abs_thumbnail_path:
            thumbnail_result = self.upload_thumbnail_with_retry(
                youtube, video_id, abs_thumbnail_path
            )
        else:
            print("  ⚠️ No thumbnail available, skipping thumbnail upload")
