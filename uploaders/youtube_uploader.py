...
class YouTubeUploader:
    ...
    def _credentials_from_cache(self, cached_data: Dict) -> Optional[Credentials]:
        """Create Credentials from cached data — FIX H6: Now attempts refresh for expired tokens"""
        try:
            ...
            if not credentials.expired:
                ...
                # FIX H6: Try to refresh expired credentials instead of returning None
                print(f"  ⏰ Cached token expired at {credentials.expiry} — attempting refresh...")
                try:
                    from google.auth.transport.requests import Request
                    credentials.refresh(Request())
                    print(f"  ✅ Token refreshed successfully")
                    # Save refreshed credentials back to cache
    ...
    def _create_upload_request(self, youtube, video_path: str, body: Dict, 
        ...
        request = youtube.videos().insert(
            ...
            body=body,
            media_body=media,
            notifySubscribers=False  # FIX H5: Changed from True to prevent notification spam for Shorts
        )
        
        # Store upload state — upload_id will be populated after first next_chunk() call
        # FIX M9: Don't try to read _upload_id here, it's only set after next_chunk()
    ...
    def upload_video(self, video_path: str, thumbnail_path: str, title: str,
        ...
        # FIX 2026: Only add #Shorts if truly missing from BOTH title AND description.
        # The orchestrator already adds #Shorts to descriptions — don't duplicate.
        # Duplicate hashtags = spam signal for YouTube's 2026 algorithm.
        has_shorts_tag = '#shorts' in description.lower() or '#shorts' in title.lower()
        ...
        while response is None:
            ...
            try:
                ...
                # FIX M9: Capture upload_id after first chunk for resume capability
                if hasattr(request, 'media_body') and request.media_body:
                    upload_id = getattr(request.media_body, '_upload_id', None)
                    if upload_id and self._upload_state.get('upload_id') is None:
                        self._upload_state['upload_id'] = upload_id
                if status:
