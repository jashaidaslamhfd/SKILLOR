...
class InstagramUploader:
    ...
    def _is_valid_public_url(self, url: str, check_reachable: bool = True, 
                              max_redirects: int = 3, _depth: int = 0) -> bool:
        """
        ...
        Args:
            ...
            _depth: FIX H11 — recursion depth counter to prevent infinite redirect loops
        """
        # FIX H11: Depth check to prevent infinite redirect loops
        if _depth > max_redirects:
            logger.warning(f"Max redirect depth ({max_redirects}) exceeded for URL: {url[:80]}...")
            return False
        ...
        try:
            ...
            if response.status_code in [301, 302, 303, 307, 308]:
                ...
                if redirect_url:
                    return self._is_valid_public_url(redirect_url, check_reachable=True, max_redirects=max_redirects, _depth=_depth+1)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 400:
                    ...
                    except (json.JSONDecodeError, ValueError, AttributeError):  # FIX M6: Specific exceptions instead of bare except
                        pass
    ...
    def _publish_container(self, container_id: str) -> Dict:
        ...
        if 'id' in result:
            # FIX H9: Instagram reel URLs use shortcode, not raw media ID.
            # The media ID format is usually "{media_id}_{user_id}" — extract the media part.
            # We attempt to fetch the permalink from the API for the correct URL.
            media_id = result['id']
            permalink = None
            try:
                fields_url = f"{self.base_url}/{media_id}?fields=permalink&access_token={self.access_token}"
                perm_result = self._rate_limited_request(fields_url, method='GET')
                if isinstance(perm_result, dict) and 'permalink' in perm_result:
                    permalink = perm_result['permalink']
            except Exception:
                pass
            
            reel_url = permalink or f"https://instagram.com/reel/{media_id.split('_')[0]}"
    ...
    def optimize_caption(self, caption: str, hashtags: List[str]) -> str:
        """Optimize caption for Instagram algorithm — FIX H10: Instagram limit is 2200 chars, not 100"""
        # Clean hashtags
        clean_tags = []
        ...
        # FIX H10: Instagram caption limit is 2200 chars, use 500 for optimal engagement
        max_caption_len = 500
