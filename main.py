...
class YouTubeAutomationSystem:
    ...
    async def create_video(self, topic_data):
        ...
        # FIX (retention 80%+): Replace passive hook with curiosity-driven opening
        for seg in script_data['segments']:
            if seg.get('type') == 'hook':
                old_hook = seg.get('text', '')
                new_hook = generate_retention_hook(topic)
                seg['text'] = new_hook
                print(f"    [RETENTION] Hook upgraded: '{old_hook[:40]}...' → '{new_hook[:40]}...'")
                break
