...
try:
    ...
    class CloudUploader:  # noqa: F811
        def __init__(self): pass
        def is_configured(self): return False
        def upload_video(self, *a, **kw): return None
...
# AUDIENCE MATCH: Hook templates for USA/UK Males 35-54
# These make a 40-year-old man STOP scrolling.
# Key: Personal statement, age-relevant, NOT a question, NOT "shocking"
RETENTION_HOOKS = [
    "After 35 your brain starts doing this and nobody warns you.",
    "Nobody tells men what {topic} actually does to your body.",
    "Your brain has been doing this with {topic} your entire adult life.",
    "The thing you believe about {topic} is quietly working against you.",
    "Doctors don't explain what {topic} really does after 40.",
    "Right now your body is doing something unexpected because of {topic}.",
    "This is what {topic} silently does to men over 35.",
    "You've experienced {topic} a thousand times but never knew this part.",
]

def generate_retention_hook(topic: str) -> str:
    """
    AUDIENCE MATCH: Bold personal statement hooks for 35-54 male.

    Rules for this audience:
    - Start with statement, NEVER a question ("Did you know" = swipe)
    - Age-relevant language ("after 35", "men", "adult life")
    - Calm and credible — not hype, not horror
    - Max 15 words — lands in 3 seconds at calm pace
    """
    import random
    template = random.choice(RETENTION_HOOKS)
...
class YouTubeAutomationSystem:
            ...
            if duration < 35:
                ...
                for seg in script_data['segments']:
                    if seg.get('type') == 'story':
                        seg['text'] += f" And maybe that's the quiet part worth sitting with... how something so small can carry so much of who you are."
                ...
                if len(story_segments) > 1:
                    ...
                    # FIX: previously this branch silently did nothing when
                    # only ONE story segment existed -- the trim "attempt"
                    # ran twice and changed nothing both times (exactly the
                    # log symptom reported: same 166 words, 3 identical
                    # attempts). Trim the text of that single segment
                    # directly instead of requiring a second segment to pop.
                    idx = story_segments[-1]
                    seg = script_data['segments'][idx]
                    words = seg['text'].split()
                    keep = max(10, int(len(words) * 0.7))
                    seg['text'] = ' '.join(words[:keep])
                    seg['duration'] = round(len(seg['text'].split()) / (AUDIO_CONFIG.WORDS_PER_MINUTE / 60), 2)
                    print(f"    📝 Trimmed story segment: {len(words)} → {keep} words")
    ...
    def _split_long_segments(self, segments: List[Dict], max_words: int = 15) -> List[Dict]:
        """FIX (root cause of 'lots of color gradient, very few real clips'):
        footage_fetcher fetches exactly ONE clip per script segment, no
        matter how long that segment's spoken text is. The 'story' section
        (~55-65 words, often 8-15s of audio) was staying as ONE segment, so
        it got exactly ONE Pexels clip -- video_assembler then carved that
        single clip into several "cuts" internally, but they're all the
        SAME source footage, and if that clip is short the remainder gets
        padded with a slow-motion stretch / animated color background
        instead of new real footage. Splitting any long speech segment into
        ~max_words-sized chunks BEFORE footage fetching means each chunk is
        treated as its own segment and gets its OWN independent clip search
        -- this is what actually produces visible "more clips" variety
        instead of one clip stretched to fill time. Pause segments and
        already-short segments pass through untouched.
        """
        ...
        for seg in segments:
            ...
            for i in range(0, len(words), chunk_size):
                ...
                new_segments.append({
                    **seg,
                    'text': chunk_text,
                    'duration': round(len(chunk_words) / wps, 2),
                })
        return new_segments
    ...
    async def create_video(self, topic_data):
        ...
        # Hook is already optimized by ContentGenerator with topic-specific AI generation
        # (10-14 words, warm curiosity tone for USA/UK audience)

        # FIX: split long speech segments (e.g. the ~55-65 word 'story'
        # section) into smaller chunks BEFORE footage fetching, so each
        # chunk gets its own independently-searched clip instead of one
        # clip covering the whole segment. See _split_long_segments
        # docstring for the full root-cause explanation.
        script_data['segments'] = self._split_long_segments(script_data['segments'], max_words=15)
        ...
        self.caption_gen.generate_karaoke_ass(audio_data['word_timings'], ass_path, max_duration=actual_duration)
            ...
            try:
                ...
                for platform, result in upload_results.items():
                    if not isinstance(result, dict):
                        print(f"  ⚠️  {platform.title()}: unexpected result type ({type(result).__name__}): {result}")
                        continue
                    ...
                    if status in ['uploaded', 'published']:
                        ...
                        print(f"  🚫 {platform.title()}: blocked by quality gate — {result.get('reason', '')}")
                    else:
