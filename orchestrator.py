...
from core.audio_genrator import AudioGenerator
...
for d in [AUDIO_DIR, FOOTAGE_DIR, VIDEO_DIR, THUMB_DIR, CAPTIONS_DIR]:
    d.mkdir(exist_ok=True)

# Fixed output paths (workflow uploads these)
...
class AutomationOrchestrator:
    ...
    async def _process_topic(self, topic: str, topic_data: Dict,
        ...
        logger.info(f"   ✅ Script: {script['word_count']} words | Hook score: {script['hook_score']}/10")

        # ── 2. AUDIO ───────────────────────────────────────────
        logger.info("🎙️ Step 2: Generating audio...")
        audio_path = str(AUDIO_DIR / f"{safe}_{ts}.mp3")
        audio_data = await self.audio_gen.generate_with_effects(
            script_segments=script['segments'],
            output_path=audio_path,
            topic=topic
        )
        logger.info(f"   ✅ Audio: {audio_data.get('duration', 0):.1f}s → {audio_path}")

        # ── 3. FOOTAGE ─────────────────────────────────────────
        logger.info("🎬 Step 3: Fetching footage...")
        footage_dir = str(FOOTAGE_DIR / f"{safe}_{ts}")
        Path(footage_dir).mkdir(exist_ok=True)
        ...
        ass_path = str(CAPTIONS_DIR / f"{safe}_{ts}.ass")
        self.caption_gen.generate_captions(
            audio_data=audio_data,
            output_path=ass_path
        )
        logger.info(f"   ✅ Captions saved")
        ...
        # Copy to fixed output paths for workflow upload steps
        ...
        if skip_upload:
            logger.info("⏭️ Skipping upload (--skip-upload flag)")
            return {
                'topic': topic,
        ...
        # ── 7. CLOUD UPLOAD (for Instagram public URL) ─────────
        video_url = None
        if self.cloud_uploader.is_configured():
            logger.info("☁️ Step 7: Uploading to cloud...")
            video_url = self.cloud_uploader.upload_video(video_path)
            logger.info(f"   ✅ Cloud URL: {video_url}")
        else:
        ...
        logger.info("📘 Step 9: Uploading to Facebook...")
        try:
    ...
    def _build_description(self, topic: str, script: Dict) -> str:
        ...
        lines = [
            f"{hook}",
            "",
            f"{story[:300]}..." if len(story) > 300 else story,
            "",
