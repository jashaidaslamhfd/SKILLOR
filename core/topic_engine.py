...
class ViralTopicEngine:
    ...
    def __init__(self):
        ...
        # FIX: Disk-backed used_topics — survives GitHub Actions fresh runners
        self._used_file = "state/used_topics.txt"
        self.used_topics = self._load_used_topics()
        print(f"📋 {len(self.used_topics)} used topics loaded from history")
    ...
    def _load_used_topics(self) -> set:
        """Load used topics from disk — key fix for GitHub Actions fresh runners"""
        try:
            os.makedirs("state", exist_ok=True)
            if os.path.exists(self._used_file):
                with open(self._used_file, 'r') as f:
                    topics = set(line.strip() for line in f if line.strip())
                # Keep last 50 to avoid permanent blacklist
                if len(topics) > 50:
                    topics = set(list(topics)[-50:])
                return topics
        except Exception as e:
            print(f"⚠️ Could not load used topics: {e}")
        return set()

    def _save_used_topic(self, topic: str):
        """Immediately write topic to disk"""
        try:
            os.makedirs("state", exist_ok=True)
            with open(self._used_file, 'a') as f:
                f.write(topic.strip() + "\n")
        except Exception as e:
    ...
    def get_daily_topics(self, count: int = 1) -> List[Dict]:
        """Get daily topics — FIX: never repeats, persists across GitHub Actions runs"""
        random.seed()
        ...
        # FIX: Shuffle BEFORE filtering — "forget names" won't always win
        random.shuffle(unique)

        fresh_topics = [t for t in unique if t['query'] not in self.used_topics]

        if not fresh_topics:
            print("♻️ All topics used — resetting history")
            self.used_topics.clear()
            try:
                if os.path.exists(self._used_file):
                    os.remove(self._used_file)
            except Exception:
                pass
            fresh_topics = unique
        ...
        # FIX: Save to disk so next GitHub Actions run remembers
        for t in result_topics:
            self.used_topics.add(t['query'])
            self._save_used_topic(t['query'])
            print(f"   📌 Topic: {t['query']}")
