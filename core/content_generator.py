...
class ContentGenerator:
    ...
    def __init__(self):
        ...
        self.client = None
        self.hook_engine = HookEngine(use_cache=False)  # FIX: was True, must be False for hook variety
        self._init_client()
    ...
    def generate_title(self, topic: str) -> str:
        ...
        # ✅ FIX C3: Removed dead code — the first return was unreachable.
        # Now using topic-aware fallbacks that were previously dead code.
    ...
    def generate_seo(self, topic: str, script: str) -> Dict:
        """Generate DYNAMIC SEO description and tags — no two videos identical.
        
        FIX 2026: Previous version generated the EXACT SAME tags and description
        for every video — YouTube's spam detector flags identical tag sets across
        videos. Now generates topic-specific tags with random variation.
        """
        # Dynamic description — topic-specific, not generic
        description_intros = [
            f"The science behind {topic} explained simply.",
            f"Why {topic} happens — and what it means for your body.",
            f"Your body does {topic} for a reason. Here's the science.",
        ...
        # Dynamic tags — topic-specific + rotating base tags
        topic_words = [w for w in topic.lower().split() if len(w) > 3]
        base_tag_sets = [
            ["body science", "brain facts", "explained"],
            ["body mysteries", "brain science", "why does my body"],
            ["health facts", "why do i feel", "explained simply"],
            ["science explained", "body facts", "your body"],
        ]
        base_tags = random.choice(base_tag_sets)
        ...
        # Dynamic keywords
        keyword_parts = topic_words[:3] + ["explained", "science"]
