...
class AudioGenerator:
    ...
    async def _generate_speech(self, text: str, path: str, rate: str) -> Tuple[float, List[Dict]]:
        ...
        # ── HUMANIZE: make AI voice sound natural ──────────────
        # 1. Micro tempo variation (±1.5%) — breaks robotic rhythm
        # 2. Subtle formant warmth (bass boost 80-200Hz)
        # 3. Room reverb (short 40ms tail) — sounds recorded in a room
        # 4. Gentle compression — evens out AI's too-perfect levels
        import random
        tempo_var = round(random.uniform(0.984, 1.016), 4)   # ±1.6%
        
        human_path = path.replace('.mp3', '_human.mp3')
        result = subprocess.run([
            'ffmpeg', '-y', '-i', path,
            '-af', (
                # Tempo micro-variation (not pitch shift — keeps natural)
                f"atempo={tempo_var},"
                # Warm up the voice — boost 100-180Hz slightly
                "equalizer=f=120:t=o:w=80:g=2.5,"
                # Cut harsh AI sibilance at 6-8kHz
                "equalizer=f=7000:t=o:w=2000:g=-2.5,"
                # Short room reverb — aecho (50ms, 0.4 decay)
                "aecho=0.8:0.6:40:0.25,"
                # Gentle compression — humanizes too-perfect TTS dynamics
                "acompressor=threshold=0.4:ratio=2.5:attack=8:release=120:makeup=1.1,"
        ...
        # If humanize fails, original TTS is still at path — safe fallback
        # ───────────────────────────────────────────────────────
