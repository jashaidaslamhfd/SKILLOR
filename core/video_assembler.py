"""
Video Assembler - FORCED ENGAGEMENT (USA 2026)
FIXES:
1. ✅ First segment ALWAYS uses footage (no blank/color clips)
2. ✅ Color grading (vibrant, engaging)
3. ✅ Fast cuts (1.2-1.6s) for retention
4. ✅ Swipe stopper on first 0.8s (flash + text)
5. ✅ Dynamic zoom on every cut
6. ✅ YouTube Shorts optimized (42-55s)
"""
...
class VideoAssembler:
    ...
    def __init__(self):
        ...
        # ── Color grading ──
        self.saturation = 1.3    # ═══ Vibrant colors ═══
        self.contrast = 1.15     # ═══ Pop ═══
        self.brightness = 0.05   # ═══ Slightly brighter ═══
        
        print(f"🎬 VideoAssembler (BABY NICHE)")
...
"""
    ...
    def create_video(self, script_segments: List[Dict], audio_data: Dict,
        ...
        """Create final video - FORCED ENGAGEMENT"""
        ...
        try:
            ...
            for i, seg in enumerate(script_segments):
                ...
                # ═══════════════════════════════════════════════════════════
                # FIX 1: FIRST SEGMENT ALWAYS USES FOOTAGE (no blank clips)
                # ═══════════════════════════════════════════════════════════
                ...
                # ── If first segment, force footage usage ──
                if is_first_segment:
                    # Try to find any footage for first segment
                    for idx in range(len(script_segments)):
                        candidate = footage_paths.get(idx)
                ...
                if not used_real_footage:
                    # Fallback: plain color with gradient
                    out = os.path.join(temp_dir, f"seg_fallback_{i}.mp4")
                    cmd = [
                        "ffmpeg", "-y", "-f", "lavfi",
            ...
            if prepad_duration > 0 and prepad_duration < target_duration - 0.2:
                ...
                if os.path.exists(video_padded):
