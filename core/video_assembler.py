...
class RetentionVideoAssembler:
    ...
    def _build_portrait_filter(self, idx: int, width: int, height: int, crop_needed: bool) -> str:
        """
        🥇 Fix 2: Resolves Ken Burns zoompan frame rate desync loops.
        Ensures portrait orientation scaling and center positioning calculations logic.
        """
        ...
        # 🚀 INJECTING FIX 2 STRUCTURES: Enforcing explicit fps specifications to eliminate video freeze anomalies
        zoom_speed = 0.0012
        zoom_filter = (
            f"zoompan=z='zoom+{zoom_speed}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
            f"d=300:s=1080x1920:fps={self.fps}"
        )
    ...
    def assemble_final_video(self, processed_clips: Dict[int, str], audio_path: str, 
        ...
        try:
            ...
            for idx in sorted_indices:
                ...
                p_filter = self._build_portrait_filter(idx, w, h, crop)
