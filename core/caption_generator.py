...
class CaptionGenerator:
    ...
    def generate_karaoke_ass(
        ...
        # FIX: Ensure first word starts at EXACTLY 0.0s — critical for swipe rate.
        # If there's any delay before the first caption, the viewer swipes before
        # reading anything. Captions must be visible from the very first frame.
        if word_timings and word_timings[0].get('start', 0) > 0.0:
            delay = word_timings[0].get('start', 0)
            logger.warning(f"⚠️ First caption delayed by {delay:.2f}s — shifting all timings to start at 0.0s")
            for wt in word_timings:
