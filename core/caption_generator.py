"""
Audio Generator - PRODUCTION READY (FIXED)
FIXES:
1. edge-tts import moved to top ✅
2. Async event loop conflict resolved ✅
3. Proper error logging ✅
4. Fallback audio generation ✅
5. Voice generation guaranteed ✅
"""
...
import edge_tts  # FIX: Import at top

from config.settings import AUDIO_CONFIG


class AudioGenerator:
    """Production Audio Generator - FIXED"""
    ...
    # ============================================================
    # TOPIC-MATCHED MUSIC (Brown Noise - NO SINE TONES)
    # ============================================================

    def _add_bg_music_and_fan(self, speech_path: str, output_path: str,
                              total_duration: float, topic: str = ""):
        """Brown-noise music bed — completely replaces sine tones"""
        topic_lower = topic.lower()

        if any(w in topic_lower for w in ['memory', 'forget', 'brain']):
            lp, hp, amp = 220, 60, 0.032
        elif any(w in topic_lower for w in ['sleep', 'tired']):
            lp, hp, amp = 160, 40, 0.026
        elif any(w in topic_lower for w in ['stress', 'anxiety']):
        ...
        if os.path.exists(fan_path) and os.path.exists(music_path):
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                print("    ✅ Voice + music bed mixed")
                return

        shutil.copy(speech_path, output_path)
        print("    ⚠️ Mix failed — speech only")
    ...
    async def _generate_speech(self, text: str, path: str, rate: str) -> Tuple[float, List[Dict]]:
        ...
        for attempt in range(1, 4):
            try:
                # FIX: edge-tts imported at top now
    ...
    async def generate_with_effects(self, script_segments: List[Dict],
        ...
        print(f"    ✅ Audio: {total_duration:.1f}s | {len(all_word_timings)} word timings")
