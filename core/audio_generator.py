...
class AudioGenerator:
    ...
    def _add_bg_music_and_fan(self, speech_path: str, output_path: str, total_duration: float):
        ...
        if os.path.exists(fan_path) and os.path.exists(music_path):
            subprocess.run([
                ...
                '-i', music_path,
                '-filter_complex',
                '[0:a]volume=1.0[speech];[1:a]volume=0.10[fan];[2:a]volume=0.03[music];'
    ...
    async def generate_with_effects(self, script_segments: List[Dict], output_dir: str) -> Dict:
        ...
        Generates TTS audio for each non-pause segment, assembles them with
        pause gaps into one final MP3, and returns word_timings with correct
        absolute timestamps.

        Uses a single authoritative timeline_cursor that advances by the
        EXACT requested segment duration (never by the rounded ffmpeg output
        duration) to prevent timestamp drift across segments.
        """
