...
class AudioGenerator:
    ...
    def _clean_text_for_tts(self, text: str) -> str:
        """
        FIX: Edge TTS unnatural breath problem.

        Edge TTS breathes at EVERY comma, semicolon, dash, and period.
        This causes wrong breaths mid-sentence:
          Bad:  "Your brain, [BREATH] dreaming because you [BREATH] are human"
          Good: "Your brain dreaming because you are human. [BREATH]"

        Fix: Remove all mid-sentence punctuation before sending to TTS.
        Only keep sentence-ending periods so breath only happens at sentence end.
        """
        import re

        # ... (3 dots) → single period (our intended breath marker)
        text = re.sub(r"\.{2,}", ".", text)

        # Mid-sentence commas before lowercase = remove (wrong breath trigger)
        text = re.sub(r",\s+(?=[a-z])", " ", text)

        # Commas before conjunctions = always mid-sentence = remove
        text = re.sub(
            r",\s+(and|but|or|so|because|when|while|after|before|which|that|who|where|how|if)",
            r" ", text, flags=re.IGNORECASE
        )

        # Semicolons, em dash, en dash = breath triggers = remove
        text = text.replace(";", "").replace("—", " ").replace("–", " ")

        # Parentheses content = remove (causes double breath)
        text = re.sub(r"\(.*?\)", "", text)

        # Clean multiple spaces
        text = re.sub(r"  +", " ", text).strip()

        # Ensure ends with period so TTS knows sentence boundary
    ...
    async def _generate_speech(self, text: str, path: str, rate: str) -> tuple:
        ...
        # FIX: Clean text before TTS — removes mid-sentence punctuation
        # that causes Edge TTS to breathe in wrong places
