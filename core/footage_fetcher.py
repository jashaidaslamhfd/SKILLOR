...
class FootageFetcher:
    ...
    def __init__(self):
        ...
        # 🐛 FIX (root cause of 70%+ repeated/swapped clips): this file was
        # never rotated. Pexels/Pixabay return only ~15-25 results per
        # keyword, so after a handful of runs EVERY result for a common
        # query (e.g. "baby smiling") was already in used_ids. That forced
        # the fallback path below to always grab the same `videos[0]`,
        # which is exactly why the same clips kept reappearing constantly.
        # Keeping only the most recent N ids gives old clips a chance to
        # recirculate instead of permanently blacklisting a tiny pool.
    ...
    def _load_used_ids(self) -> set:
        try:
            ...
            if os.path.exists(self.used_ids_file):
                ...
                # Only keep the most recent window so old clips recirculate
        ...
        try:
            with self._state_lock:
                with open(self.used_ids_file, 'a') as f:
                    f.write(clip_id + "\n")
                # 🐛 FIX: rewrite/rotate the file periodically so it never
                # grows forever and never permanently exhausts small pools.
    ...
    def search_pixabay(self, query: str) -> List[Dict]:
        ...
        params = {"key": self.pixabay_key, "q": query, "per_page": 25, "orientation": "vertical"}
        try:
    ...
    def fetch_footage_for_script(self, script_segments: List[Dict], topic: str) -> List[Dict]:
        ...
        for i, segment in enumerate(script_segments):
            ...
            if not selected_video and videos:
                # 🐛 FIX: was always `videos[0]` -> same clip got reused on
                # nearly every segment/run once the pool ran low. Pick
                # randomly among the candidates instead so repeats are at
                # least spread out rather than being the identical clip.
