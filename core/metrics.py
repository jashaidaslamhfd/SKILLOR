...
class MetricsTracker:
    ...
    def _load_metrics(self):
        ...
        if self.metrics_file.exists():
            ...
            except (json.JSONDecodeError, OSError, ValueError):  # FIX M7: Specific exceptions instead of bare except
    ...
    def record_video(self, video_data: Dict, upload_results: Dict):
        ...
        # FIX M1: Daily stats now accumulate instead of overwrite
        today = datetime.now().strftime('%Y-%m-%d')
        daily_entry = {
            'date': today,
            'videos': [video_data.get('topic', '')],
            'total_duration': video_data.get('duration', 0),
            'total_hook_score': video_data.get('hook_score', 0),
            'total_word_count': video_data.get('word_count', 0),
            'video_count': 1,
            'uploads': {k: [v] for k, v in upload_results.items()}
        }
        
        # Check if today already exists — accumulate instead of overwrite
        existing = [d for d in self.data['daily_stats'] if d.get('date') == today]
        if existing:
            entry = existing[0]
            entry['videos'].append(video_data.get('topic', ''))
            entry['total_duration'] += video_data.get('duration', 0)
            entry['total_hook_score'] += video_data.get('hook_score', 0)
            entry['total_word_count'] += video_data.get('word_count', 0)
            entry['video_count'] = entry.get('video_count', 1) + 1
            for platform, result in upload_results.items():
                if platform not in entry['uploads']:
                    entry['uploads'][platform] = []
                entry['uploads'][platform].append(result)
        else:
            self.data['daily_stats'].append(daily_entry)
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'success_rate': (
                self.data['successful_uploads'] / max(1, self.data['successful_uploads'] + self.data['failed_uploads']) * 100
            ),  # FIX M2: Success rate = uploads_succeeded / total_upload_attempts (not total_videos)
            'average_duration': (
